apparent = function (InputFile, MaxIdent=0.10, alpha=0.01, nloci=300, self=TRUE, plot=TRUE, files=TRUE, Dyad=FALSE, dir) {
  ...
  if (files==TRUE){
    if(!missing(dir)){
      write.csv(Triad_All, file.path(dir, "apparent-Triad-All.csv"),row.names=F)
    }
  }
  ...
  if (files==TRUE) {
    blank <- data.frame(" "," "," "," "," "," ")
    if(!missing(dir)) {
      write.table(LogOut1,file.path(dir,"apparent-Triad-Summary.csv"),sep=",",col.names=T,row.names=F)
      write.table(blank,file.path(dir,"apparent-Triad-Summary.csv"),sep=",",col.names=F,row.names=F,append=T)
      write.table(blank,file.path(dir,"apparent-Triad-Summary.csv"),sep=",",col.names=F,row.names=F,append=T)
      write.table(LogOut2,file.path(dir,"apparent-Triad-Summary.csv"),sep=",",col.names=T,row.names=F,append=T)
    }
  } 
  ...
  if (TDtGap$p.value < alpha ) {
    ...
    if (nrow(L) > 0) {    
      ...
      if (files==TRUE){
        if(!missing(dir)){
          write.csv(Triad_Sig,file.path(dir, "apparent-Triad-Sig.csv"),row.names=F)
        }
      }
    }
  } else {
  ...
  if (plot==TRUE & files==FALSE) {
    ...
    png(file.path(dir,"apparent-Triad-Plot.png"), width=6,height=6,units='in',res=400)
  ...
  if (Dyad==TRUE) {
    ...
    if (nrow(OffsD) < 1) {
      ...
      if (nrow(Out3) < 1) {
        ...
        Dyad_Sig <- Out3
        assign("Dyad_Sig", Out3, envir = envir)
        Out3b <- Out3
        if (files==TRUE){
          if(!missing(dir)){
            write.csv(Out3,file.path(dir, "apparent-Dyad-Sig.csv"),row.names=F)
          }
        }
        ...
        if (files==TRUE){
          if(!missing(dir)){
            write.csv(Dyad_Sig,file.path(dir, "apparent-Dyad-Sig.csv"),row.names=F)
          }
        }
      }
      ...
      if ( plot==TRUE & files==TRUE & nrow(Out3b > 0) ) {
        pdf(file.path(dir,"apparent-Dyad-Plot.pdf"),width=7,height=4)
